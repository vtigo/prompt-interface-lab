  ...
  return (
    <div className="flex flex-col bg-zinc-100 h-screen text-zinc-700">
      <div className='flex flex-col w-full max-w-4xl mx-auto h-full mt-6 p-4 overflow-hidden'>
        ...
        <div id="chat" className="flex-1 overflow-y-auto p-4 mb-4 min-h-0 max-h-full">
            ...
            <div className="space-y-12">
              {messages.map((message) => (
                message.role === 'user' ? (
                  ...
                  <SystemMessage key={message.id} message={message} messageFiles={messageFiles} isLoading={isLoading} />
                )
              ))}
            </div>
          )}
...
const UserMessage = ({ message }: { message: UIMessage }) => {
  return (
    <div className="flex w-full min-w-full justify-end">
      <div className="px-5 py-3 max-w-[80%] rounded-2xl border border-zinc-300">
  ...
  return (
    <div className="flex w-full min-w-full justify-start">
      <div className="w-full">
        ...
        {messageFiles[message.id] && !isLoading && (
          <div className="mt-2">
...
const MessagesInfos = ({ messages }: { messages: UIMessage[] }) => {
  return (
    <div className="flex-1 flex flex-col overflow-y-scroll max-h-1/2 text-xs p-3 border bg-yellow-50">
      <div className="font-semibold mb-3">Message Debug Info:</div>
      {messages.map(message => (
